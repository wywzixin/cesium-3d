...
export function MVTProvider(options) {
    ...
    this._styleFun = Cesium.defined(options.styleFun)
        ? options.styleFun
    ...
    this._resolutions = (mapExtent, 22, this._tileWidth);
    this._pixelRatio = 1;
...
function createMapboxStreetsV6Style() {
    ...
    let polygon = new ol.style.Style({ fill });
    let strokedPolygon = new ol.style.Style({ fill, stroke });
    let line = new ol.style.Style({ stroke });
