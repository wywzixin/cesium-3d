...
class DrawHelper {
    ...
    startMeasureHeight(options, callbacks) {
        this.drawingMeasureHeightshape(options, callbacks);
    }
    ...
    drawingMeasureHeightshape(options = {}, callbacks = {}) {
        this.startDrawing(function() {
            options.primitives.remove(poly);
            if (markers !== null) {
                markers.destroy();
        ...
        let poly;
        let minPoints = 2;
        poly = new MeasureHeight(options);
        poly.asynchronous = false;
        options.scene = scene;
        options.primitives.add(poly);
        let positions = [];
            ...
            if (movement.position != null) {
                ...
                if (cartesian) {
                    if (positions.length >= minPoints) {
                        ...
                        // positions.push(cartesian);
                        // options.scene = scene;
                        // primitive = new MeasureHeight(options);
                        // primitive.setPositions(positions);
                        // addDeleteListener(primitive);
                        // this.measureCollection.push(primitive);
                        // options.primitives.add(primitive);
                        // if (typeof callbacks.startEditCallback === 'function') {
                        //     primitive.startEditCallback = callbacks.startEditCallback;
                        // }
                    ...
                    if (positions.length === 0) {
                        ...
                        positions.push(cartesian.clone());
                        positions.push(cartesian.clone());
                        // verticalPositions.push(cartesian.clone());
                        // verticalPositions.push(cartesian.clone());
                        markers.addBillboard(positions[0]);
                        // cartesian.y += 1 + Math.random();
                        markers.addBillboard(cartesian);
                        // markers.addBillboard(verticalPositions[1]);
                    }
                }
            }
        }, ScreenSpaceEventType.LEFT_CLICK);
            ...
            if (position != null) {
                if (positions.length !== 0) {
                    ...
                    if (cartesian) {
                        ...
                        // cartesian.y += 1 + Math.random();
                        // positions.push(cartesian.clone());
                        positions[1] = cartesian.clone();

                        // 算第三个点
                        let horizonalCoords = this.computesHorizontalLine(positions);
                        positions[2] = horizonalCoords.clone();
                        // 高度
                        // verticalPositions.pop();
                        // verticalPositions.push(horizonalCoords.clone());
                        // 水平线
                        // horizonalPositions = [];
                        // horizonalPositions.push(cartesian.clone());
                        // horizonalPositions.push(horizonalCoords.clone());
                        poly.positions = positions;
                        // poly.horizonalPositions = horizonalPositions;
                        // poly.verticalPositions = verticalPositions;
                        poly._createPrimitive = true;
                        // if (positions.length >= minPoints) {
    ...
    computesHorizontalLine(positions) {
        // 计算第三个点
